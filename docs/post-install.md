...
- If you aren't aware, disabling Spectre and Meltdown in the [InSpectre](https://www.grc.com/inspectre.htm) program sets the value of the registry keys ``FeatureSettingsOverride`` and ``FeatureSettingsOverrideMask`` to ``3``. According to Microsoft's documentation, this is a bitmask in which each bit represents whether a certain mitigation is enabled (0) or disabled (1). Theoretically, you could disable *all* mitigations by setting all bits to 1, but the effects of doing so have not been extensively evaluated. This would result in a value of ``4294967295`` (32-bit DWORD). Of course to enable certain mitigations (e.g. Meltdown for the reasons mentioned previously), the relevant bits must be set to 0. Once again, this is not a recommendation, it is theoretical
