...
- If you have more than one XHCI controller, you can isolate devices such as your mouse, keyboard and audio devices (if any) onto another controller to [prevent them from interfering with polling consistency](https://forums.blurbusters.com/viewtopic.php?f=10&t=7618#p58449). Other controllers may be made available by using the ports connected to the front panel header or an external PCIe card that offers XHCI ports. Always verify with [USB Device Tree Viewer](https://www.uwe-sieber.de/usbtreeview_e.html)
